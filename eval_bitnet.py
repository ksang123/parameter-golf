#!/usr/bin/env python3
"""Standalone eval script for BitNet ternary models. Supports DDP.
...
Usage:
  ...
  # 8 GPU:
  torchrun --standalone --nproc_per_node=8 eval_bitnet.py --model final_model.ternary.ptz --ttt --ttt-all
"""
...
def load_model(path: str, args, device):
    data = open(path, "rb").read()
    for decompress in [lzma.decompress, zlib.decompress]:
        try:
            raw = decompress(data)
            break
        except Exception:
...
def main():
    ...
    parser.add_argument("--ttt-all", action="store_true")
    ...
    log0(f"Model loaded from {cli.model}", rank)

    # Sliding window (skip if running TTT since TTT includes it)
    if not cli.ttt and not cli.ttt_all:
        ...
        torch.cuda.synchronize()
        log0(f"val_loss:{slide_loss:.6f} val_bpb:{slide_bpb:.6f} time:{time.perf_counter()-t0:.1f}s", rank)

    if cli.ttt:
        pre_sd = {k: v.clone() for k, v in model.state_dict().items()}
        log0("\n=== TTT (frozen ternary) ===", rank)
        torch.cuda.synchronize()
        t0 = time.perf_counter()
        ...
        torch.cuda.synchronize()
        log0(f"val_loss:{ttt_loss:.6f} val_bpb:{ttt_bpb:.6f} time:{time.perf_counter()-t0:.1f}s", rank)
